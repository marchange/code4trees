...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const authDropdown    = document.getElementById('authDropdown');       // eingeloggt: Profil/Logout
  const authDropdownGuest = document.getElementById('authDropdownGuest'); // ausgeloggt: Einloggen/Registrieren
  ...
  function applyLoggedInUser(user) {
    ...
    // Auf index.html: Login-Gate weg, Formular sichtbar, Username read-only anzeigen.
    const submitAsUsername = document.getElementById('submitAsUsername');
    const submitLoginGate  = document.getElementById('submitLoginGate');
    const submitForm       = document.getElementById('submitForm');
    if (submitAsUsername && user.username) submitAsUsername.textContent = user.username;
    if (submitLoginGate) submitLoginGate.hidden = true;
    ...
    if (authDropdownGuest) authDropdownGuest.hidden = true;
  }
  ...
  // Klick auf den Nav-Button togglet immer genau ein Dropdown:
  // eingeloggt -> #authDropdown (Profil/Logout), ausgeloggt -> #authDropdownGuest (Einloggen/Registrieren).
  if (authNavBtn) {
    authNavBtn.addEventListener('click', (e) => {
      const target = authNavBtn.dataset.loggedIn === 'true' ? authDropdown : authDropdownGuest;
      if (!target) return; // z.B. login.html/registrieren.html ohne Guest-Dropdown: normal weiter-navigieren
      e.preventDefault();
      const isOpen = !target.hidden;
      // Immer beide schließen, dann ggf. das Ziel wieder öffnen (sauberer Zustand).
  ...
  document.addEventListener('click', (e) => {
    if (!authNavWrapper || authNavWrapper.contains(e.target)) return;
    if (authDropdown) authDropdown.hidden = true;
    if (authDropdownGuest) authDropdownGuest.hidden = true;
  });
  ...
  // "Einloggen" im Guest-Dropdown öffnet das Modal (index.html) bzw. navigiert zu login.html.
  if (openLoginBtn) {
  ...
  // Login-Gate im Einreichen-Bereich: gleiche "Einloggen öffnen"-Logik wie im Nav-Dropdown.
  ...
  // zu registrieren.html schicken, egal was sonst noch mit Modal-Klicks passiert.
